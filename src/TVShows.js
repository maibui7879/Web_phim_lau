    ...
    try {
      ...
      const updatedData = await Promise.all(
        response.data.data.items.map(async (item) => {
          const tvShowDetailResponse = await axios.get(
            `https://phimapi.com/phim/${item.slug}`
          );
          const tvShow = {
            ...item,
            poster_url: tvShowDetailResponse.data.movie.poster_url,
          };
          return tvShow;
        })
      );
      setTvShows(updatedData);
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-6">
        {tvShows.map((show) => (
          <div
            ...
            onClick={() => handleTvShowClick(show.slug)}
          >
            <img
              src={show.poster_url}
              alt={show.name}
            ...
            />
            <h2 className="text-xl font-semibold mt-4 truncate">{show.name}</h2>
            <p className="text-sm text-gray-400">{show.origin_name}</p>
            <p className="text-sm text-gray-400">Năm: {show.year}</p>
          </div>
        ))}
      </div>
